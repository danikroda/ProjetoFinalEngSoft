...
# 1. A INTERFACE DO PADRÃO COMMAND
class Comando(ABC):
...
# 2. OS COMANDOS CONCRETOS (Cada um cuida apenas da sua própria tela)

class ComandoCadastrar(Comando):
...
class ComandoMovimentar(Comando):
    def __init__(self, controlador: EstoqueController):
        self.__controlador = controlador
...
class ComandoAlternarStatus(Comando):
    def __init__(self, controlador: EstoqueController):
        self.__controlador = controlador
